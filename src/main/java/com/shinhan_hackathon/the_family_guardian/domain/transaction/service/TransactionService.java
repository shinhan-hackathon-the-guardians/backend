...
public class TransactionService {
	...
	public void updateDeposit(DepositRequest depositRequest) {
		...
		accountService.updateAccountDeposit(depositRequest.accountNumber(), depositRequest.transactionBalance());
		User user = findByAccountNo(depositRequest.accountNumber());
	...
	public void updateWithdrawal(WithdrawalRequest withdrawalRequest) {
		...
		User user = findByAccountNo(withdrawalRequest.accountNumber());

		TransactionStatus transactionStatus = null;
		PaymentLimitType paymentLimitType = null;
		// TODO: Rule Check
		if (paymentLimitService.checkMaxAmountLimit(user.getId(), withdrawalRequest.transactionBalance())) {
			if (paymentLimitService.checkSingleTransactionLimit(user.getId(),
				withdrawalRequest.transactionBalance())) { // 승인
				accountService.updateAccountDeposit(user.getAccountNumber(),
					withdrawalRequest.transactionBalance()); // 출금
				// TODO: 출금 성공 알림
		...
		switch (transactionStatus) {
			case APPROVE -> fcmSender.sendWithdrawalSuccessMessage(user.getDeviceToken(), user.getAccountNumber(),
				withdrawalRequest.transactionBalance());
			case REJECT -> fcmSender.sendWithdrawalFailMessage(user.getDeviceToken(), user.getAccountNumber(),
				withdrawalRequest.transactionBalance());
	...
	public void updateTransfer(TransferRequest transferRequest) {
		...
		User user = findByAccountNo(transferRequest.withdrawalAccountNumber());
		String withdrawalAccountNumber = user.getAccountNumber();

		TransactionStatus transactionStatus = null;
		PaymentLimitType paymentLimitType = null;
		// TODO: Rule Check
		if (paymentLimitService.checkMaxAmountLimit(user.getId(), transferRequest.transactionBalance())) {
			if (paymentLimitService.checkSingleTransactionLimit(user.getId(),
				transferRequest.transactionBalance())) { // 승인
				accountService.updateAccountTransfer(transferRequest.depositAccountNumber(), withdrawalAccountNumber,
					transferRequest.transactionBalance()); // 이체
		...
		switch (transactionStatus) {
			case APPROVE -> fcmSender.sendTransferSuccessMessage(user.getDeviceToken(), withdrawalAccountNumber,
				transferRequest.depositAccountNumber(), transferRequest.transactionBalance());
			case REJECT -> fcmSender.sendTransferFailMessage(user.getDeviceToken(), withdrawalAccountNumber,
				transferRequest.depositAccountNumber(), transferRequest.transactionBalance());
			case PENDING -> sendGuardianApprovalMessage(user, transaction, paymentLimitType);
		}
	}
	...
	public void updatePayment(PaymentRequest paymentRequest) {
		...
		switch (transactionStatus) {
			case APPROVE -> fcmSender.sendPaymentSuccessMessage(user.getDeviceToken(), user.getAccountNumber(),
				paymentRequest.businessName(), paymentRequest.transactionBalance());
			case REJECT -> fcmSender.sendPaymentFailMessage(user.getDeviceToken(), user.getAccountNumber(),
				paymentRequest.businessName(), paymentRequest.transactionBalance());
			case PENDING -> sendGuardianApprovalMessage(user, transaction, paymentLimitType);
		}
	}
		...
		transactionList.stream()
			...
			.forEach(transaction -> {
				...
				switch (transaction.getTransactionType()) {
					case WITHDRAWAL ->
						fcmSender.sendWithdrawalFailMessage(user.getDeviceToken(), user.getAccountNumber(),
							transaction.getTransactionBalance());
					case TRANSFER -> fcmSender.sendTransferFailMessage(user.getDeviceToken(), user.getAccountNumber(),
						transaction.getReceiver(), transaction.getTransactionBalance());
					case PAYMENT -> fcmSender.sendPaymentFailMessage(user.getDeviceToken(), user.getAccountNumber(),
						transaction.getReceiver(), transaction.getTransactionBalance());
				}
			});
	...
	public void executeWithdrawalTransaction(WithdrawalApproveEvent event) {
		...
		if (transaction.getApproveCount() >= approvalRequirement) { // 승인 요구치에 도달하면
			...
			fcmSender.sendWithdrawalSuccessMessage(user.getDeviceToken(), user.getAccountNumber(),
				transaction.getTransactionBalance());
	...
	public void executeTransferTransaction(TransferApproveEvent event) {
		...
		if (transaction.getApproveCount() >= approvalRequirement) { // 승인 요구치에 도달하면
			// 이체
			accountService.updateAccountTransfer(transaction.getReceiver(), user.getAccountNumber(),
				transaction.getTransactionBalance());
			transaction.updateTransactionStatus(TransactionStatus.APPROVE);
			fcmSender.sendTransferSuccessMessage(user.getDeviceToken(), user.getAccountNumber(),
				transaction.getReceiver(), transaction.getTransactionBalance());
			log.info("Success to execute transfer. [Transaction-ID: {}]", transaction.getId());
		} else if ((user.getFamily().getTotalManagerCount() - transaction.getRejectCount()) < approvalRequirement) {
			//  거부 알림
			transaction.updateTransactionStatus(TransactionStatus.REJECT);
			fcmSender.sendTransferFailMessage(user.getDeviceToken(), user.getAccountNumber(),
	...
	public void executePaymentTransaction(PaymentApproveEvent event) {
		...
		if (transaction.getApproveCount() >= approvalRequirement) { // 승인 요구치에 도달하면
			...
			accountService.updateAccountWithdrawal(user.getAccountNumber(), transaction.getTransactionBalance());
			transaction.updateTransactionStatus(TransactionStatus.APPROVE);
			fcmSender.sendPaymentSuccessMessage(user.getDeviceToken(), user.getAccountNumber(),
				transaction.getReceiver(), transaction.getTransactionBalance());
			log.info("Success to execute payment. [Transaction-ID: {}]", transaction.getId());
		} else if ((user.getFamily().getTotalManagerCount() - transaction.getRejectCount()) < approvalRequirement) {
			//  거부 알림
			transaction.updateTransactionStatus(TransactionStatus.REJECT);
			fcmSender.sendPaymentFailMessage(user.getDeviceToken(), user.getAccountNumber(), transaction.getReceiver(),
				transaction.getTransactionBalance());
	...
	private User findByAccountNo(String accountNumber) {
